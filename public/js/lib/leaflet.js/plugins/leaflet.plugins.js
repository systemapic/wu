...
L.Note = L.Handler.extend({
		...
		initialize: function (thisParent, map, latLngBounds, options, rectangle) {
				 ...
				 this._popupPane = map._panes.popupPane;
				 this._name = 'L.Note';
				 this._initialZoom = rectangle._initialZoom || map.getZoom();
				 ...
				 L.Handler.prototype.initialize.call(this, map, options); // was L.Draw.Simpleshape...
		...
		onAdd: function (map) {
				...
				this._el.onmousemove = function (e) {
						...
						if (map._popup) { map._popup._close() };
	    ...
	    _reset: function () {
				...
				// update layer's size
				var ne = map.latLngToLayerPoint(this._latLngBounds._northEast);
		        var sw = map.latLngToLayerPoint(this._latLngBounds._southWest);
				...
				// set zoomOffset
				this._zoomOffset = this._initialZoom - map.getZoom(); // either 0, -1 or +1 (if ±2, note not shown)
...
/*
				if (this._zoomOffset > 0) { 
					//this._fontScale--;
					this._fontScale = (this.options.fontSize / (this._fontScale) * (-1)) + 'em';
					console.log('fs2: ', this._fontScale);
				} else {
				//	this._fontScale++;
					this._fontScale = (this.options.fontSize * (this._fontScale)) + 'em';
//					this._fontScale = (this.options.fontSize * 2) + 'em';
					console.log('fs3: ', this._fontScale);
				}
*/
		...
		_fireCreatedEvent: function (layer) {
				//this._map.fire('draw:note:box_created', { layer: layer, layerType: this.type });
		},
...
L.Edit.Rectangle = L.Edit.SimpleShape.extend({
        ...
        _onMarkerDragStart: function (e) {
        		console.log('_onMarkerDragStart');
        ...
        _resize: function (latlng) {
                ...
                if (this._shape._note) {
						...
						var corners = this._getCorners();
						var noteTopLeft = map.latLngToLayerPoint(corners[0]);
						var noteBottomRight = map.latLngToLayerPoint(corners[2]);
...
L.EditToolbar.Edit = L.Handler.extend({
        ...
        enable: function () {
                ...
                console.log('map.LeafletDrawEditEnabled = true 22');
				map.LeafletDrawEditEnabled = true;
                L.Handler.prototype.enable.call(this);
        ...
        disable: function () {
				...
				console.log('map.LeafletDrawEditEnabled = false 3');
				//map.LeafletDrawEditEnabled = false;
        ...
        _enableLayerEdit: function (e) {
        		...
        		console.log('map.LeafletDrawEditEnabled = true 5');
        		map.LeafletDrawEditEnabled = true;
        ...
        _disableLayerEdit: function (e) {
        		...
        		console.log('map.LeafletDrawEditEnabled = false 8');
        		//map.LeafletDrawEditEnabled = false;
...
L.EditToolbar.Delete = L.Handler.extend({
        ...
        _removeLayer: function (e) {
 				...
 				if (e.target._note) {
					console.log('_removeLayer e._note');
		...
		_removeAllLayers: function (e) {
				console.log('_removeAllLayers:');
				console.log(e);
				console.log(this);
				for (layer in map.myDrawnItems._layers) {
					console.log(layer);
					
					ui.drawControl._deletableLayers.removeLayer(map.myDrawnItems._layers[layer]);
					ui.drawControl._deletedLayers.addLayer(map.myDrawnItems._layers[layer]);
				}
...
L.Draw.Feature.prototype.enable = function () {
		...
		console.log('map.LeafletDrawEditEnabled = true 9');
		map.LeafletDrawEditEnabled = true;
