...
function startColorThemes() {
	...
	function initBegin() {
		...
		eVars._color_Outer_wrapper.style.height = '172px';		
	
	
		// Clickers
	
		// DARK COLOR THEMES
		eVars._dark1.onclick = function() {prethemes('one');}
		eVars._dark2.onclick = function() {prethemes('two');}
		eVars._dark3.onclick = function() {prethemes('three');}
	...
	var	initClassArray = {
			...
			stackClasses : function (theClassArray) {
				...
				theClassArray[11] = ['.ct11',
									 '.editor-projects-item:hover', '.smap-button-gray', '.smap-button-white',
									 '.item-list.controls-item.active', 
									 '.active .layers-mapbox-item',
									 '.item-list.active', '.layer-item.active'
									 ];
				...
				theClassArray[18] = ['.ct18', '.smap-button-gray', '.smap-button-white', '.item-list.controls-item.active', 
...
//			preval_14 = 'rgba(0,0,0,0.7)';		// Background color on Map Boxes
	...
	//			preval_14 = 'rgba(0,0,0,0.7)';		// Background color on Map Boxes
		...
		if ( colorTheme.cThemeDarkHeader ) {
			colorTheme.css[15].value = 'black';
			colorTheme.css[16].value = 'white';			
		} else {
			colorTheme.css[15].value = 'white';	
	...
	init : function() {
		...
		if ( !colorTheme.cTheme ) prethemes('one');
...
//		var _hex = eVars._hexColor.value;
	...
	colorTheme.css[17].value = 'rgba(255, 255, 255, 0.15)';	// Background color Boxes on sidepane (New client box, etc)
	...
	colorTheme.css[13].value = '#000000';					// Text color on Main Menu Shade 2 (Plus the white buttons, but that's not right...)
	
	colorTheme.css[17].value = rgba_25;					// Bacground color Buttons			
	...
	colorTheme.css[24].value = rgba_25;					// Search Fields
