...
module.exports = api = {
	...
	zipAndSend : function (req, res) {
		...
		console.log('API: zipAndSend => ');
		console.log('files: ', files);
		console.log('puuids: ', puuids);
		console.log('pslig: ', pslug);
		...
		fs.mkdirs(maindir, function (err) {		// refactor
			...
			async.each(files, function (file, callback) {
				...
				fs.copy(src, dest, function(err){ 
					if (err) {
						console.error('copyu err: ', err);
						console.log('trying /images/');
						var src = false;
						if (file.data) {
							if (file.data.image) {
								if (file.data.image.file) {
									src = '/var/www/data/images/' + file.data.image.file;
								}
							}
						}
						...
						if (!src) {
							...
							fs.copy(src, dest, function (err) {
								if (err) console.log('second err', err);
								callback(err);
							})
						}
					} else {
			...
			function (err) {	// todo: err handling
				...
				console.log('API: Download Files => zipping! cmd: ', cmd);	
